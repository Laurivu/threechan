...
float old_pot0, old_pot1, old_pot2, old_pot3, pot0, pot1, pot2, pot3, logpot3, old_logpot3, pot4, old_pot4, pot5, old_pot5, channel, old_channel, echokill, old_echokill;
...
void MyCallback(float **in, float **out, size_t size)
{
  ...
  for (size_t i = 0; i < size; i++)
  {
    ...
    // Output
    out[1][i] = del_out * mute;
    out[0][i] = del_out * mute;
...
void setup() {
  ...
  // set all channels on by default
  //pinMode(CHN1, OUTPUT);
  //pinMode(CHN2, OUTPUT);
  //pinMode(CHN3, OUTPUT);
  analogWrite(CHN1, 255);
  analogWrite(CHN2, 255);
  analogWrite(CHN3, 255);
...
void loop() {
  ...
  // Delay filter pot
  pot1 = (map(analogRead(A2), 0.0, 1023.0, 350.0, 6000.0));
  ...
  // Channel switcher
  channel = analogRead(A3);
  if (abs(channel - old_channel) > 100){
    old_channel = channel;
    if (channel < 250) {
    kanava = 1;
    analogWrite(CHN1, 255);
    ...
    Serial.println(kanava);
  }

  // Echo mute
  echokill = analogRead(A7);
  if (abs(echokill - old_echokill) > 100) {
    old_echokill = echokill;
    if (echokill < 300) {
      mute = 0.01;
    }
    else if (echokill < 600) {
      mute = 0.6;
    }
    else {
      mute = 1.0;
    }
    Serial.print("echokill value: ");
    Serial.println(echokill);
    Serial.println(mute);
